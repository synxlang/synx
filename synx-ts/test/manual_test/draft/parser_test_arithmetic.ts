...
const Minus = mkCharSeq("-");
const Star = mkCharSeq("*");
const Slash = mkCharSeq("/");
const LeftParen = mkCharSeq("(");
const RightParen = mkCharSeq(")");

const NumberLiteral: PatternSeq = mkPatternSeq([Digit], "+", true);
const AddOp = mkPatternSet([Plus, Minus]);
const MulOp = mkPatternSet([Star, Slash]);

const Expr: PatternSet = mkPatternSet([], undefined, [LeftParen, RightParen], Space);
const Factor: PatternSet = mkPatternSet([]);
const ProductTail: PatternSeq = mkPatternSeq([MulOp, Factor], "  ", false, null, false, Space);
const Product: PatternSeq = mkPatternSeq([Factor, ProductTail], " *", false, null, false, Space);
const SumTail: PatternSeq = mkPatternSeq([AddOp, Product], "  ", false, null, false, Space);
const Sum: PatternSeq = mkPatternSeq([Product, SumTail], " *", false, null, false, Space);
...
function parseCaseRoot(src: string): { parser: ParserImpl; result: ASTNode | null } {
...
function evalProductTail(node: ASTNode): { op: string; value: number } {
  const [opSlot, factorSlot] = node.raw_value as [ASTNode | null, ASTNode | null];
...
function evalProduct(node: ASTNode): number {
  const [firstSlot, tailSlot] = node.raw_value as [ASTNode | null, ASTNode[] | null];
...
function evalSumTail(node: ASTNode): { op: string; value: number } {
  const [opSlot, productSlot] = node.raw_value as [ASTNode | null, ASTNode | null];
