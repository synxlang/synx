 ...
 */
const LineDelimiter = mkPatternSet([CrLf, Lf]);

/**
 * Comment = (CommentPrefix, comment:AnyChar*, LineDelimiter?) => comment
 * AnyChar with `*` is normalized to non-greedy by `mkPatternSeq`.
 * AnyChar 搭配 `*` 会被 `mkPatternSeq` 规范为非贪婪。
 */
const Comment: PatternSeq = mkPatternSeq([CommentPrefix, AnyChar, LineDelimiter], " *?");
const MutiConmments: PatternSeq = mkPatternSeq([Comment], "*", false, null, false, AnyChar); // ignore Anychar
