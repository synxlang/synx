import { matchChar, completeCharRange, completeCharSet } from '../../src/parser_matcher';
...
const DIGIT_RANGE = completeCharRange({ start: '0', end: '9' });
const DIGITS = completeCharSet({ sub_nodes: '0123456789' });
const AZ_RANGE = completeCharRange({ start: 'a', end: 'z' });
const AZ_UP = completeCharRange({ start: 'A', end: 'Z' });
const US = completeCharSet({ sub_nodes: '_' });
const NL = completeCharSet({ sub_nodes: '\n' });
const CR = completeCharSet({ sub_nodes: '\r' });
const EMOJI_RANGE = completeCharRange({ start: '😀', end: '😀' });
const DIGIT_SET = completeCharSet({ sub_nodes: [DIGIT_RANGE] });
const AZ_SET = completeCharSet({ sub_nodes: [AZ_RANGE] });
const NESTED_DIGITS = completeCharSet({ sub_nodes: [DIGITS] });
const LETTER_OR_UNDERSCORE = completeCharSet({ sub_nodes: [AZ_RANGE, AZ_UP, US] });
const TRIPLE_L3 = completeCharSet({ sub_nodes: '0123456789' });
const TRIPLE_L2 = completeCharSet({ sub_nodes: [TRIPLE_L3] });
const TRIPLE_L1 = completeCharSet({ sub_nodes: [TRIPLE_L2] });
const WHITESPACE = completeCharSet({ sub_nodes: ' \t\v\n' });
const SEMICOLON = completeCharSet({ sub_nodes: ';' });
const NL_OR_CR = completeCharSet({ sub_nodes: [NL, CR] });
const EMOJI_SET = completeCharSet({ sub_nodes: '😀' });
const EMOJI_RANGE_SET = completeCharSet({ sub_nodes: [EMOJI_RANGE] });
/** CJK Unified Ideographs Basic Block: U+4E00 (一) to U+9FFF */
const CJK_RANGE = completeCharRange({ start: '\u4E00', end: '\u9FFF' });
const CJK_SET = completeCharSet({ sub_nodes: [CJK_RANGE] });
/** Several Chinese characters in \oneof form */
const CHINESE_SAMPLE = completeCharSet({ sub_nodes: '中文' });
/** Character set containing both single-code-unit (a, 中) and double-code-unit (😀) characters */
const MIXED_SINGLE_AND_SURROGATE = completeCharSet({ sub_nodes: 'a中😀' });
/** Range from 一(U+4E00) to 😀(U+1F600), covering BMP single-code-unit and supplementary plane double-code-unit characters */
const MIXED_RANGE = completeCharRange({ start: '\u4E00', end: '😀' });
const MIXED_RANGE_SET = completeCharSet({ sub_nodes: [MIXED_RANGE] });
const DIGIT_OR_LETTER = completeCharSet({ sub_nodes: [DIGITS, AZ_SET] });
const DIGIT_OR_LETTER_R = completeCharSet({ sub_nodes: [LETTER_OR_UNDERSCORE, DIGIT_SET] });
/** AnyChar as a single child node */
const ANYCHAR_SET = completeCharSet({ sub_nodes: [AnyChar] });
/** AnyChar combined with other nodes - AnyChar placed last as fallback option */
const DIGIT_OR_ANYCHAR = completeCharSet({ sub_nodes: [DIGIT_RANGE, AnyChar] });
const LETTER_OR_ANYCHAR = completeCharSet({ sub_nodes: [AZ_RANGE, AnyChar] });
...
function test_completeCharSet_validates_non_empty(): void {
    assert.throws(() => completeCharSet({ sub_nodes: '' }), /sub_nodes must not be empty/);
    assert.throws(() => completeCharSet({ sub_nodes: [] }), /sub_nodes must not be empty/);
}
function run_all_tests(): void {
    console.log('Running match_char tests...\n');
    try {
        test_completeCharSet_validates_non_empty();
        test_match_char();
        console.log('\n✅ All tests passed!');
    }
    catch (error) {
