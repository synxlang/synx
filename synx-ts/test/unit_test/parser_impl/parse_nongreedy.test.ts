...
const Seq_A_star_B_mandatory_nongreedy = completePatternSeq({ sub_nodes: [A, B], sub_quantifiers: "* ", raw: false, sep: null, accept_trailing_sep: false, ignore: null, greedy_flags: [false, true] });
const Seq_A_q_B_q_C_mandatory = completePatternSeq({ sub_nodes: [A, B, C], sub_quantifiers: "?? ", raw: false, sep: null, accept_trailing_sep: false, ignore: null, greedy_flags: [false, false, true] });
const Seq_A_star_B_star_C_mandatory = completePatternSeq({ sub_nodes: [A, B, C], sub_quantifiers: "** ", raw: false, sep: null, accept_trailing_sep: false, ignore: null, greedy_flags: [false, false, true] });
const Seq_A_star_B_plus_C_mandatory = completePatternSeq({ sub_nodes: [A, B, C], sub_quantifiers: "*+ ", raw: false, sep: null, accept_trailing_sep: false, ignore: null, greedy_flags: [false, false, true] });
const Seq_A_plus_Five_mandatory_comma = completePatternSeq({ sub_nodes: [A, Five], sub_quantifiers: "+ ", raw: false, sep: Comma, accept_trailing_sep: false, ignore: null, greedy_flags: [false, true] });
const Seq_A_star_B_mandatory_ignoreSpace = completePatternSeq({ sub_nodes: [A, B], sub_quantifiers: "* ", raw: false, sep: null, accept_trailing_sep: false, ignore: Space as ParserNode, greedy_flags: [false, true] });
const Seq_AnyChar_star_Bang = completePatternSeq({ sub_nodes: [AnyChar, Bang], sub_quantifiers: "* " });
const Seq_AnyChar_plus_Bang = completePatternSeq({ sub_nodes: [AnyChar, Bang], sub_quantifiers: "+ " });
const Seq_AnyChar_star_Digit = completePatternSeq({ sub_nodes: [AnyChar, Digit], sub_quantifiers: "* " });
...
function test_parsePatternSeq_nongreedy(): void {
    const cases: NongreedyCase[] = [
        ...
        },
        {
            ...
            expected: mkSeqAST(Seq_A_plus_Five_mandatory_comma, [0, 5], [[mkCharSeqAST(A, "a", [0, 1]), mkCharSeqAST(A, "a", [2, 3])], mkCharSeqAST(Five, "5", [4, 5])], [mkCharSeqAST(Comma, ",", [1, 2]), mkCharSeqAST(Comma, ",", [3, 4])]),
            expected_error: false,
        },
        {
            id: 6,
